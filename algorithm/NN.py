    ...
    def _set_nn(self, weight=-1, init=False):
        if weight == 1:
            if init:
                ...
                w = prep.read_nn_from_file('SMGANN_2022-04-07T17-30-35_W1_relu-softmax-6-10-None-3_1000-2000.csv')
        elif weight == 2:
            if init:
                w = np.random.rand(self.layer_1_size, self.output_size)
            else:
                w = prep.read_nn_from_file('SMGANN_2022-04-07T17-30-35_W2_relu-softmax-6-10-None-3_1000-2000.csv')
        else:
            pass
